...
class EnhancedReFrameworkGenerator():
	def __init__(self):
		#Set Up A Generator
		self.enhanced_reframework_generator = Generator("UiPath Enhanced REFramework Generator", 
		...
		messages = ["Welcome to " + self.enhanced_reframework_generator.name + "!", "By Christian Blandford", "github: cblandford"]
		terminal.header(messages, "yellow", "yellow", "=")

		self.set_up_scaffolding()

	#The code we will upload for production
	def set_up_scaffolding(self):
		...
		if should_scaffold_files == "Y":
			...
			if should_pass_config == "Y":
				self.edit_scaffolded_files(self.enhanced_reframework_generator.files_created, True)
			else:
				self.edit_scaffolded_files(self.enhanced_reframework_generator.files_created, False)
	...
	def edit_scaffolded_files(self, files_created, should_pass_config=False):
...
def main():
	gen = EnhancedReFrameworkGenerator()
